...
# 再额外打开新加的开关：
#   adv_loss_soft_clip: True
# 含义：使用 Polynomial Decay Soft Clip。
# 当 ratio 接近 1+eps (或 1-eps) 时，梯度平滑衰减至 0，而不是像 PPO 那样硬截断。
#
...
for env_id in "${atari_envs[@]}"; do
  echo "============================"
  echo "Starting env: $env_id (PPO mean+std + adv_loss_soft_clip, 4 seeds, GPUs 0/1)"
  echo "============================"
  ...
  for i in "${!seeds[@]}"; do
    ...
    run_name="ppo_meanStd_advLossSoftClip"
...
echo "All Atari PPO mean+std + adv_loss_soft_clip runs finished."

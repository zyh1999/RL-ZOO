...
# 使用 PPO 默认 Atari 超参（ppo.yml 中的 atari 段），并显式开启 advantage 归一化：
...
for env_id in "${atari_envs[@]}"; do
  echo "============================"
  echo "Starting env: $env_id (PPO default hyperparams + adv norm mean+std, 4 seeds, GPUs 0/1)"
  echo "============================"
...
echo "All Atari PPO mean+std runs finished."
